...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # Main area (table)
        self.results_table = QTableWidget(0, 4, central)
    ...
    def _connect_signals(self) -> None:
        ...
        # Ensure correct initial state (Auto default).
    ...
    def _on_scan_mode_changed(self) -> None:
        ...
        # All other modes are auto-filled and read-only.
        self.scan_range_input.setReadOnly(True)
        self.scan_all_checkbox.setVisible(mode == "Auto")

        if mode == "Auto":
            detected = self._detect_local_cidr()
            self.scan_range_input.setText(detected)
            return
        ...
        # Preset CIDR-ish strings (kept exactly as shown in the dropdown).
    ...
    def _parse_subnets_for_scan(self) -> list[str]:
        ...
        for part in raw.split(","):
            ...
            # Accept UI range-like forms (e.g. 192.168.1.1–254) as /24.
            token = token.replace("–", "-")
    ...
    def _trigger_scan(self) -> None:
        # Prevent duplicate starts while a scan is already running.
    ...
    def _show_results_context_menu(self, pos) -> None:
        ...
        # Force-select the row that was right-clicked (even if another row was selected).
        try:
        ...
        # Snapshot row data now to avoid any race with live insertions.
        try:
    ...
    def _on_scan_finished_with_count(self, count: int) -> None:
        # Prefer worker's count as authoritative.
    ...
    def _set_initial_column_widths(self) -> None:
        # Prefer sane defaults; user can resize.
        self.results_table.setColumnWidth(0, 220)  # Name
        self.results_table.setColumnWidth(1, 140)  # IP
        self.results_table.setColumnWidth(2, 170)  # MAC
        self.results_table.setColumnWidth(3, 260)  # Vendor
